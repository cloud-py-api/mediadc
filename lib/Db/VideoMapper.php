...
class VideoMapper extends QBMapper {
	...
	public function findAllResolvedByUser(string $userId, int $limit = null, int $offset = null) {
		...
		$qb->select(
			...
			->from($this->tableName, 'mdc_videos')
			->innerJoin('mdc_videos', 'filecache', 'ocf', 'ocf.fileid=mdc_videos.fileid')
			...
			->where(
				...
				$qb->expr()->gte('mdc_videos.skipped', $qb->createNamedParameter(100, IQueryBuilder::PARAM_INT))
			)
			->groupBy('ocf.fileid')
			->setMaxResults($limit)
			->setFirstResult($offset);
