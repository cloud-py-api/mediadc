    ...
    def _request_authentication(self):
        ...
        if self.server_capabilities & CLIENT.CONNECT_ATTRS:
            ...
            for k, v in self._connect_attrs.items():
                k = k.encode("utf-8")
                connect_attrs += _lenenc_int(len(k)) + k    # connect_attrs += struct.pack("B", len(k)) + k
                v = v.encode("utf-8")
                connect_attrs += _lenenc_int(len(v)) + v    # connect_attrs += struct.pack("B", len(v)) + v
            data += _lenenc_int(len(connect_attrs)) + connect_attrs     # data += struct.pack("B", len(connect_attrs)) + connect_attrs
