...
def detect_pgsql_socket(config: dict, php_info: str) -> None:
    socket_path = config['dbhost'].split(":")[-1]
    if os.path.exists(socket_path):
        config['usock'] = socket_path
...
def init():
    ...
    if not php_ok:
        php_info = ''
        Warnings.append(f"Cant get php info.")
...
def close_connection(connection_id: int = 0):
    ...
    if connection.get('type', None) is not None and connection.get('impl', None) is not None:
        try:
            ...
            print(f'Exception({type(e).__name__}) when closing DB:{str(e)}')
...
def execute_fetchall(query: str, args=None, connection_id: int = 0) -> list:
    ...
    for i in range(3):
        ...
        try:
            ...
            internal_handle_db_connect_exception(e)
...
def check_db() -> list:
    ...
    try:
        ...
        ret.append(f'Exception({type(e).__name__}):{str(e)}')
    finally:
        if not success:
